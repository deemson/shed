...
fn main() -> ExitCode {
    ...
    if let Command::Config { command: ConfigCommand::Schema { command } } = &cli.command {
...
fn run(cli: &Cli) -> Result<sync::Outcome, error::Error> {
    ...
    let collection_path = collection_arg.canonicalize().map_err(|e| {
        error::Error::Io(format!("collection {:?}", collection_arg), e)
    })?;
    ...
    let config_dir = match &cli.items_dir {
        Some(dir) => dir.canonicalize().map_err(|e| {
            error::Error::Io(format!("items directory {:?}", dir), e)
        })?,
        None => collection_path
            .parent()
            .ok_or_else(|| {
                error::Error::MissingArg("cannot determine config directory from collection path".to_string())
            })?
            .to_path_buf(),
    };

    let cwd = std::env::current_dir().map_err(|e| {
        error::Error::Io("current directory".to_string(), e)
    })?;
    ...
    let (direction, sync_dir) = match &cli.command {
        Command::Put { target } => {
            let dir = match target {
                Some(d) => d.canonicalize().map_err(|e| {
                    error::Error::Io(format!("target directory {:?}", d), e)
                })?,
                None => cwd,
            };
        ...
        Command::Get { source } => {
            let dir = match source {
                Some(d) => d.canonicalize().map_err(|e| {
                    error::Error::Io(format!("source directory {:?}", d), e)
                })?,
                None => cwd,
            };
